...
app.layout = dbc.Container(
    ...
    children=[
        dbc.Row([
            dbc.Col([
                html.Label("Date Range (days back)", className="small fw-semibold text-muted"),
                dcc.Slider(id="days-sl", min=1, max=30, step=1, value=7,
...
def _synthetic_runs(days_back):
    ...
    for dag in DAG_NAMES:
        for day_offset in range(days_back):
            ...
            rows.append({
                "DAG": dag, "Start": run_start, "Finish": run_end,
                "State": state, "Duration_m": duration_m,
            })
...
def update_gantt(days_back):
    ...
    for _, row in df.iterrows():
        tasks.append(dict(
            Task=row["DAG"], Start=row["Start"].isoformat(), Finish=row["Finish"].isoformat(),
            Resource=row["State"],
        ))
    ...
    fig = ff.create_gantt(
        tasks, colors=colors, index_col="Resource", show_colorbar=True,
        group_tasks=True, title=f"Pipeline Run Timeline — Last {days_back} Day(s)",
    )
...
def update_success_rate(days_back):
    df = _synthetic_runs(days_back)
    summary = (
        df.groupby("DAG")["State"]
        .apply(lambda s: (s == "success").sum() / len(s) * 100)
        .reset_index()
    )
    ...
    fig = go.Figure(go.Bar(
        ...
        orientation="h",
        marker_color=["#198754" if v >= 90 else "#fd7e14" if v >= 70 else "#dc3545"
                      for v in summary["SuccessRate"]],
        text=[f"{v:.0f}%" for v in summary["SuccessRate"]],
        textposition="inside",
    ))
    fig.update_layout(
        title="DAG Success Rate (%)", plot_bgcolor="white",
        xaxis=dict(range=[0, 100], showgrid=True, gridcolor="#f0f0f0"),
        margin=dict(l=130, r=20, t=50, b=30), height=280,
    )
    return fig
...
def update_duration(days_back):
    ...
    for dag in DAG_NAMES:
        ...
        fig.add_trace(go.Box(
            y=sub["Duration_m"], name=dag.replace("_dag","").replace("_"," ").title(),
            boxmean=True,
        ))
    fig.update_layout(
        title="Run Duration Distribution (minutes)", plot_bgcolor="white",
        yaxis_title="Minutes", showlegend=False,
        margin=dict(l=50, r=10, t=50, b=40), height=280,
    )
