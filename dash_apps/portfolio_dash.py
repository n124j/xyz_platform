...
def update_aum_chart(days, benchmark):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=df["benchmark"] / 1e9,
            mode="lines",
            name=benchmark.upper().replace("_", " "),
...
def update_allocation(days):
    ...
    fig.update_layout(title="Asset Allocation", margin=dict(l=0, r=0, t=40, b=0), height=320)
    return fig
...
def update_attribution(days):
    ...
    fig.update_layout(
        title="Performance Attribution (bps)",
        barmode="group",
        plot_bgcolor="white",
...
def update_rolling_return(days):
    ...
    fig.update_layout(
        title="21-Day Rolling Annualised Return",
        plot_bgcolor="white",
        legend=dict(orientation="h"),
        margin=dict(l=40, r=10, t=40, b=30),
        height=280,
