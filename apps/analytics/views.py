...
class AnalyticsDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        portfolio_risk = (
            RiskMetric.objects.filter(scope="PORTFOLIO", lookback_days=252).order_by("-calculation_date").first()
        )
        ...
        metrics = list(
            RiskMetric.objects.order_by("-calculation_date").values(
                "scope",
                "reference_id",
                "calculation_date",
                "var_95_1d",
...
class MarketDataAPIView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, ticker):
        ...
        records = (
            MarketData.objects.filter(ticker=ticker.upper(), price_date__gte=since)
            .order_by("price_date")
            .values("price_date", "close_price", "volume")
        )
        data = [
            {
